...
public class StringUtils {
    ...
    public static String[] getStringArrayByPattern(String str, String pattern) {
        ...
        while (matcher.find()) {
            for (int i = 0; i < matcher.groupCount(); i++) { // int groupCount()
                // 返回当前查找所获得的匹配组的数量。
                // org.jeecgframework.core.util.LogUtil.info(matcher.groups(i));
     ...
     * @return
     */
    public static String getOrgCode(String max) {
        String split = max.substring(max.length() - 3);
        int old = Integer.parseInt(split) + 1;
        StringBuffer newBuffer = new StringBuffer(String.valueOf(old));
        int length = newBuffer.length();
        if (newBuffer.length() > 3) {
            return null;
        } else {
            for (int i = 0; i < 3 - length; i++) {
                newBuffer.insert(0, '0');
            }
        }
        return newBuffer.insert(0, max.substring(0, max.length() - 3)).toString();
    }
}
